 ...
 */
export function extractFluentChains(source) {
    const chains = [];
    ...
    //          x.AddDatabase("name")...;
    const chainRegex = /(?:(?:var|const)\s+(\w+)\s*=\s*)?(\w+)\.(Add\w+)(?:<[^>]+>)?\s*\(([^)]*)\)((?:\s*\.\s*\w+\s*\([^)]*(?:\([^)]*\)[^)]*)*\))*)\s*;/g;
    let match;
    ...
    for (const lambda of lambdaChains) {
        ...
        const parent = chains.find((c) => c.raw.includes(lambda.parentMethod) && c.raw.includes(lambda.raw));
        if (parent) {
 ...
 */
export function parseMethodChain(chainStr) {
    const methods = [];
 ...
 */
export function extractFirstStringArg(argsStr) {
 ...
 */
export function extractNamedArgs(args) {
    const result = {};
 ...
 */
function extractLambdaChains(source) {
    const results = [];
