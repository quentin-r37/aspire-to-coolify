...
import { extractFluentChains } from './tokenizer.js';
import { isDatabaseChain, extractDatabase, extractChildDatabases, isContainerChain, extractContainer, isApplicationChain, extractApplication, } from './extractors/index.js';
/**
 ...
 */
function buildReferences(app, chainMap) {
    const references = [];
...
function getConnectionStringEnvName(dbType) {
    switch (dbType) {
        case 'postgres':
            return 'DATABASE_URL';
        case 'sqlserver':
...
function validateApp(app, warnings) {
    ...
    for (const application of app.applications) {
        for (const ref of application.references) {
            const found = app.databases.some((d) => d.variableName === ref || d.name === ref) ||
